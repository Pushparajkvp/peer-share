...
class IPFSDaemon(private val androidContext: Context) {
    ...
    private fun downloadFile(activity: Activity) {
        ...
        val source = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Okio.buffer(Okio.source(activity.assets.open(getBinaryFileByABI(Build.SUPPORTED_ABIS[0]))))
        } else {
            Okio.buffer(Okio.source(activity.assets.open(getBinaryFileByABI(Build.CPU_ABI))))
        }
